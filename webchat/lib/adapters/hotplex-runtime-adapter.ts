...
import { wsUrl, workerType, apiKey, workDir, allowedTools, getApiKey, getWsUrl, type ConnectionState } from '@/lib/config';
...
export function useHotPlexRuntime({
  ...
  useEffect(() => {
    ...
    const client = new BrowserHotPlexClient({
      ...
      workerType,
      apiKey: () => getApiKey(),
      authToken: () => getApiKey(), // pass via init envelope auth.token for deferred auth
      initConfig,
      heartbeat: {
